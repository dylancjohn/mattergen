...
def smact_validity(
    ...
    Thin wrapper around `smact.screening.smact_validity` (smact>=4) that keeps the
    `comp`/`count` call signature. Oxidation states are filtered by smact's ICSD24
    statistics via `smact.screening.ICSD24FilterConfig(consensus, commonality)`. The
    default `commonality="low"` is looser than smact's own default ("medium"), which also
    drops oxidation states below 10% occurrence for their element, so results are only
    comparable between calls with the same settings.
    ...
    Args:
        ...
        commonality: Excludes oxidation states below a proportion-of-occurrence threshold for
            their element: "low", "medium", "high", "main" or a numeric threshold, as in
